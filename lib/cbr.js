module.exports = function (moment, xml2js, db, httpGet) {
  ...
  return {
    ...
    getFcMarketLib: async function() {
      ...
      const xmlData = await httpGet(requestUrl);
    ...
    getQuotesByDate : async function (date) {
      ...
      // отправляем запрос
      // TODO: обратка ошибки получения ответа (сервер выключен, ошибка соединения)
      const xmlData = await httpGet(requestURL);
      ...
      return new Promise((resolve, reject) => {
        ...
        parseString(xmlData, (err, data)=> {
          if (!err) {
            ...
            // TODO Центробанк не формирует котировки по понедельникам и праздничным дням. Выяснить, есть ли график
